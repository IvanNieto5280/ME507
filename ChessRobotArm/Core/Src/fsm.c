...
/**
 * @brief Main FSM to calibrate, control via joysticks, and Estop
 *
...
void FSM_Update(FSM_t *fsm)
{
	 switch(fsm->systemState)
	 {
	 	 ...
	 	 case SYS_CALIBRATE:
	 		 ...
	 		 X_Homing(&motor1);
	 		 //Turn off stall current for now for Z axis
	 		 TMC_SPI_Send40(0x9000061F00, &motor3);
	 		 Y_Homing(&motor2, -21000);
	 		// IHOLD_IRUN: IHOLD=10, IRUN=31 (max. current), IHOLDDELAY=6
	 		 TMC_SPI_Send40(0x9000061F0A, &motor3);
	 		 Z_Homing(&motor3, 31092);

	 		 //Set Desired Motion params based on tuning
	 		 Set_Motor_Params(&motor1);
	 		 Set_Motor_Params(&motor2);
	 		 Set_Motor_Params(&motor3);
...
/**
 * @brief Open and Close the Servo as two states
 *
 * @param *fsm Pointer to the fsm structure
 *
 * @return None.
