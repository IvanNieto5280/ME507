...
void TMC_EnableReferenceSwitches(stepper_motor_driver_t *motor)
{
     ...
     * bit 0 = stop_l_enable
     * bit 1 = stop_r_enable
     * bit 2 = pol_stop_l, active low
     * bit 3 = pol_stop_r, active low
     * bit 5 = latch_l_active
     * bit 7 = latch_r_active
     *
     * This assumes switches are wired with pullups and switch to GND.
     */

    TMC_WriteRegister(TMC_SW_MODE, 0x000000AF, motor);
}
