...
SystemState_t currentState = STATE_CLOSED;
...
#define TEST_VELOCITY      10000
#define TEST_ACCELERATION  1000
#define TEST_DECELERATION  1000
...
void Test_Two_Limit_Switches(void);
...
void Test_Two_Limit_Switches(void)
{
    ...
    HAL_UART_Transmit(&huart2,
                      (uint8_t*)"Starting two-switch test on motor2\r\n",
                      strlen("Starting two-switch test on motor2\r\n"),
                      HAL_MAX_DELAY);

    TMC_Stop(&motor2);
    HAL_Delay(100);

    TMC_ClearRampStatus(&motor2);
    HAL_Delay(20);

    TMC_ResetPosition(&motor2);
    HAL_Delay(20);

    sprintf(msg,
            "Initial XACTUAL: %ld\r\n",
            (long)TMC_GetPosition(&motor2));
    ...
    TMC_MoveVelocityRight(&motor2);

    while (!TMC_RightSwitchActive(&motor2))
    {
        if (HAL_GPIO_ReadPin(EStop_GPIO_Port, EStop_Pin) == GPIO_PIN_SET)
        {
            HAL_UART_Transmit(&huart2,
                              (uint8_t*)"EStop triggered\r\n",
        ...
        if ((HAL_GetTick() - lastPrintTime) >= 100)
        {
            currentPosition = TMC_GetPosition(&motor2);

            sprintf(msg,
                    "Moving right XACTUAL: %ld\r\n",
    ...
    rightPosition = TMC_GetPosition(&motor2);

    sprintf(msg,
            "Right switch hit. XACTUAL: %ld\r\n",
            (long)rightPosition);
    ...
    HAL_UART_Transmit(&huart2,
                      (uint8_t*)"Reversing toward left switch REFL...\r\n",
                      strlen("Reversing toward left switch REFL...\r\n"),
                      HAL_MAX_DELAY);

    lastPrintTime = HAL_GetTick();

    TMC_MoveVelocityLeft(&motor2);

    while (!TMC_LeftSwitchActive(&motor2))
    {
        if (HAL_GPIO_ReadPin(EStop_GPIO_Port, EStop_Pin) == GPIO_PIN_SET)
        {
            HAL_UART_Transmit(&huart2,
    ...
    leftPosition = TMC_GetPosition(&motor2);

    sprintf(msg,
            "Left switch hit. XACTUAL: %ld\r\n",
            (long)leftPosition);
    ...
    HAL_UART_Transmit(&huart2,
                      (uint8_t*)"Two-switch test complete. Motor stopped.\r\n",
                      strlen("Two-switch test complete. Motor stopped.\r\n"),
                      HAL_MAX_DELAY);
}
...
int main(void)
{
  ...
  HAL_GPIO_WritePin(NCS_2_GPIO_Port, NCS_2_Pin, GPIO_PIN_SET);
  HAL_GPIO_WritePin(NCS_3_GPIO_Port, NCS_3_Pin, GPIO_PIN_SET);

  TMC_Init(&motor2);

  TMC_SetMaxVelocity(&motor2, TEST_VELOCITY);
  TMC_SetMaxAcceleration(&motor2, TEST_ACCELERATION);
  TMC_SetMaxDeceleration(&motor2, TEST_DECELERATION);
...
//	  HAL_Delay(2500);
//	  TMC_SPI_Send40(0xAD00000000, &motor2);
//	  HAL_Delay(2500);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : EStop_Pin Diag1_3_Pin */
  GPIO_InitStruct.Pin = EStop_Pin|Diag1_3_Pin;
    ...
    if (GPIO_Pin == User_Button_Pin)
    {
   	 ...
   	 switch(currentState)
   	 {
			  case STATE_CLOSED:
				  ...
				  __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_3, SERVO_CLOSED);
